...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        self.conv_template.append_message(self.conv_template.roles[0], f"{self.jailbreak} {self.goal} {self.control}")
        ...
        if self.conv_template.name == 'llama-2':
            ...
            self.conv_template.update_last_message(f"{self.jailbreak}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._jailbreak_slice = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks)))

            separator = ' ' if self.jailbreak else ''
            self.conv_template.update_last_message(f"{self.jailbreak}{separator}{self.goal}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._goal_slice = slice(self._jailbreak_slice.stop, len(toks))

            self.conv_template.update_last_message(f"{self.jailbreak}{separator}{self.goal}{separator}{self.control}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            ...
            if python_tokenizer:
                ...
                self.conv_template.update_last_message(f"{self.jailbreak}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._jailbreak_slice = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks)-1))

                separator = ' ' if self.jailbreak else ''
                self.conv_template.update_last_message(f"{self.jailbreak}{separator}{self.goal}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._goal_slice = slice(self._jailbreak_slice.stop, len(toks)-1)
                ...
                self._jailbreak_slice = slice(
                    encoding.char_to_token(prompt.find(self.jailbreak)),
                    encoding.char_to_token(prompt.find(self.jailbreak) + len(self.jailbreak))
                )
...
class PromptManager(object):
    ...
    def update_best_jailbreaks(self, model):
...
class MultiPromptAttack(object):
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            if (i + 1) % selection_interval == 0:
                for prompts_manager in self.prompts:
                    prompts_manager.update_best_jailbreaks(self.models[0])
